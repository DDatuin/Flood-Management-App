...
class PlaceSearchPopup extends StatefulWidget {
  ...
  const PlaceSearchPopup({super.key, required this.mode});
...
class _PlaceSearchPopupState extends State<PlaceSearchPopup> {
  ...
  String get _title {
    switch (widget.mode) {
      case LocationMode.start:
        return "Select starting point";
      case LocationMode.end:
  ...
  Widget build(BuildContext context) {
    ...
    final showCurrentLocation =
        !isSearching && widget.mode == LocationMode.start;

    final currentLocCount = showCurrentLocation ? 1 : 0;
    final recentCount = _controller.text.isEmpty ? _recentPlaces.length : 0;
    final famousCount = _controller.text.isEmpty ? _famousPlaces.length : 0;
